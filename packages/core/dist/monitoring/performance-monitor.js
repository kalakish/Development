...
class PerformanceMonitor extends events_1.EventEmitter {
    ...
    monitoring = false;
    interval = null;
    constructor(metricsCollector, healthChecker) {
        super();
    ...
    registerThreshold(threshold) {
        this.thresholds.set(threshold.metric, threshold);
        this.emit('thresholdRegistered', threshold);
    }
    unregisterThreshold(metric) {
    ...
    async createAlert(metric, value, threshold, level) {
        ...
        const alert = {
            ...
            level,
            timestamp: new Date(),
            message: `${metric} exceeded ${level} threshold: ${value.toFixed(2)} (threshold: ${level === 'critical' ? threshold.critical : threshold.warning})`
        };
    ...
    analyzePerformance(duration = 3600000) {
        const cutoff = Date.now() - duration;
        const relevantSamples = this.samples.filter(s => s.timestamp.getTime() > cutoff);
        ...
        const alertsByLevel = this.alerts.filter(a => a.timestamp.getTime() > cutoff).reduce((acc, a) => {
    ...
    // ============ Report Generation ============
    async generatePerformanceReport(duration = 3600000) {
