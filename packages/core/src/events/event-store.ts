...
import { SQLServerProvider } from '@nova/orm/sqlserver-provider';
...
export class EventStore extends EventEmitter {
    ...
    constructor(options: EventStoreOptions) {
        super();
        this.provider = new SQLServerProvider({ connection: options.connection });
    ...
    private async ensureEventStoreTables(): Promise<void> {
    ...
    private async getOrCreateStream(streamId: string): Promise<EventStream> {
        const query = `
    ...
    private async createStream(streamId: string, streamType: string = 'default'): Promise<EventStream> {
        const query = `
    ...
    private async updateStreamVersion(streamId: string, version: number): Promise<void> {
        const query = `
    ...
    async deleteStream(streamId: string, softDelete: boolean = true): Promise<void> {
        if (softDelete) {
            const query = `
                UPDATE [${this.tablePrefix}Streams]
    ...
    private async createSnapshot(streamId: string, version: number, events: StoredEvent[]): Promise<void> {
    ...
    private createSnapshotData(events: StoredEvent[]): any {
    ...
    async findEventsByCorrelationId(correlationId: string): Promise<StoredEvent[]> {
        const query = `
            SELECT * FROM [${this.tablePrefix}Events]
    ...
    async findEventsByType(eventType: string, fromDate?: Date, toDate?: Date): Promise<StoredEvent[]> {
        let query = `
            SELECT * FROM [${this.tablePrefix}Events]
    ...
    async truncateEvents(streamId?: string): Promise<void> {
        if (streamId) {
            await this.provider.executeQuery(
                `DELETE FROM [${this.tablePrefix}Events] WHERE [StreamId] = @streamId`,
                [streamId]
            ...
            await this.provider.executeQuery(
                `UPDATE [${this.tablePrefix}Streams] SET [Version] = 0 WHERE [StreamId] = @streamId`,
                [streamId]
            );
        } else {
    ...
    async getStats(): Promise<EventStoreStats> {
        ...
        const streamCount = await this.provider.executeQuery(
            `SELECT COUNT(*) AS Count FROM [${this.tablePrefix}Streams] WHERE [DeletedAt] IS NULL`
        );
        const snapshotCount = await this.provider.executeQuery(
            `SELECT COUNT(*) AS Count FROM [${this.tablePrefix}Snapshots]`
        );
